...
① 複数mztファイル連続読み込み
　連続して読み込ませたいmztファイルのファイル名の最後に#n(n=1～9)連続する数字を付加する
例)
hoge#1.mzt
hoge#2.mzt
hoge#3.mzt
...
② 連結mzt
　連続して読み込ませたいmztファイルを単純にマージしたmztファイル
　ファイル名の最後に#を付加すること
例)
hoge#.mzt

③ 連続データブロック形式(IFB、DATA、DATA…)[ダミーヘッダ無]
　インフォーメションブロックに続き、複数のデータブロックが配置されたmztファイル
　実機CMTファイルと同様ダミーヘッダはない形式です
　ファイル名に制限なし
例)
hoge.mzt
...
　インフォーメションブロックに続き、複数のデータブロックが配置されたmztファイル
　③の形式のはずですがIFBがないと動作しないエミュレータ用にダミーヘッダが付加されたもの
　ファイル名の最後に$を付加すること
例)
hoge$.mzt
